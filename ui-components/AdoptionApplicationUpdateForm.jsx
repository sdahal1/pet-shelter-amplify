...
import { getAdoptionApplication } from "./graphql/queries";
import { updateAdoptionApplication } from "./graphql/mutations";
const client = generateClient();
export default function AdoptionApplicationUpdateForm(props) {
  const {
    applicationId: applicationIdProp,
    adoptionApplication: adoptionApplicationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = adoptionApplicationRecord
      ? { ...initialValues, ...adoptionApplicationRecord }
      : initialValues;
    setApplicationId(cleanValues.applicationId);
    setApplicantName(cleanValues.applicantName);
    setEmail(cleanValues.email);
    setPhone(cleanValues.phone);
    setSubmittedAt(cleanValues.submittedAt);
    setErrors({});
  };
  const [adoptionApplicationRecord, setAdoptionApplicationRecord] =
    React.useState(adoptionApplicationModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = applicationIdProp
        ? (
            await client.graphql({
              query: getAdoptionApplication.replaceAll("__typename", ""),
              variables: { applicationId: applicationIdProp },
            })
          )?.data?.getAdoptionApplication
        : adoptionApplicationModelProp;
      setAdoptionApplicationRecord(record);
    };
    queryData();
  }, [applicationIdProp, adoptionApplicationModelProp]);
  React.useEffect(resetStateValues, [adoptionApplicationRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateAdoptionApplication.replaceAll("__typename", ""),
            variables: {
              input: {
                applicationId: adoptionApplicationRecord.applicationId,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "AdoptionApplicationUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(applicationIdProp || adoptionApplicationModelProp)}
